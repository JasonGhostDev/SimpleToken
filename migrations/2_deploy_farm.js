  ...
  try {
    ...
    if (configs.lpExist) {
      if (!configs.Farm) {
        ...
        await deployer.deploy(Farm, dataParse['SimpleToken'], web3.utils.toBN(configs.farm_param.rewardPerBlock), startBlock, {
          gas: 5000000
        });
        ...
        if (configs.farm_param.fund) {
          const simpleTokenInstance = await SimpleToken.at(dataParse['SimpleToken']);
          await simpleTokenInstance.approve(Farm.address, web3.utils.toBN(configs.farm_param.fund));
